...
///
/// \file   OLSRPO.cc
/// \brief  Implementation of OLSRPO agent and related classes.
///
/// This is the main file of this software because %OLSRPO's behaviour is
/// implemented here.
///
...
/// \param arg OLSRPO agent passed for a callback.
///

Define_Module(OLSRPO);


std::ostream& operator<<(std::ostream& os, const OLSRPO_rt_entry& e)
{
...
OLSRPO::GlobalRtable OLSRPO::globalRtable;
OLSRPO::DistributionPath OLSRPO::distributionPath;

/********** Timers **********/

void OLSRPO_Timer::removeTimer()
{
    removeQueueTimer();
}

OLSRPO_Timer::OLSRPO_Timer(OLSRPO* agent) : cOwnedObject("OLSRPO_Timer")
{
    agent_ = agent;
    tuple_ = nullptr;
}

OLSRPO_Timer::~OLSRPO_Timer()
{
    removeTimer();
}

OLSRPO_Timer::OLSRPO_Timer() : cOwnedObject("OLSRPO_Timer")
{
    agent_ = dynamic_cast <OLSRPO*> (this->getOwner());
    if (agent_==nullptr)
        throw cRuntimeError("timer ower is bad");
    tuple_ = nullptr;
}

void OLSRPO_Timer::removeQueueTimer()
{
...
void OLSRPO_Timer::resched(double time)
{
    removeQueueTimer();
    agent_->timerQueuePtr->insert(std::pair<simtime_t, OLSRPO_Timer *>(simTime()+time, this));
    //if (this->isScheduled())
...
///
void
OLSRPO_HelloTimer::expire()
{
    agent_->send_hello();
    // agent_->scheduleAt(simTime()+agent_->hello_ival_- JITTER,this);
    agent_->timerQueuePtr->insert(std::pair<simtime_t, OLSRPO_Timer *>(simTime() + agent_->hello_ival() - agent_->jitter(), this));
}
...
///
void
OLSRPO_TcTimer::expire()
{
    if (agent_->mprselset().size() > 0)
        agent_->send_tc();
    // agent_->scheduleAt(simTime()+agent_->tc_ival_- JITTER,this);
    agent_->timerQueuePtr->insert(std::pair<simtime_t, OLSRPO_Timer *>(simTime() + agent_->tc_ival() - agent_->jitter(), this));
...
//  agent_->scheduleAt(simTime()+agent_->mid_ival_- JITTER,this);
    agent_->timerQueuePtr->insert(std::pair<simtime_t, OLSRPO_Timer *>(simTime() + agent_->mid_ival() - agent_->jitter(), this));
#endif
}
...
///
/// The task of actually removing the tuple is left to the OLSRPO agent.
///
/// \param e The event which has expired.
///
...
void
OLSRPO_DupTupleTimer::expire()
{
    OLSRPO_dup_tuple* tuple = dynamic_cast<OLSRPO_dup_tuple*> (tuple_);
    ...
    else
    {
        ...
        agent_->timerQueuePtr->insert(std::pair<simtime_t, OLSRPO_Timer *>(simTime()+DELAY_T(time), this));
    }
}

OLSRPO_DupTupleTimer::~OLSRPO_DupTupleTimer()
{
    removeTimer();
    if (!tuple_)
        return;
...
OLSRPO_LinkTupleTimer::OLSRPO_LinkTupleTimer(OLSRPO* agent, OLSRPO_link_tuple* tuple) : OLSRPO_Timer(agent)
{
    tuple_ = tuple;
...
void
OLSRPO_LinkTupleTimer::expire()
{
    double now;
    now = SIMTIME_DBL(simTime());
    OLSRPO_link_tuple* tuple = dynamic_cast<OLSRPO_link_tuple*> (tuple_);
    if (tuple->time() < now)
    {
        removeTimer();
        delete this;
    ...
    else if (tuple->sym_time() < now)
    {
        ...
        agent_->timerQueuePtr->insert(std::pair<simtime_t, OLSRPO_Timer *>(simTime()+DELAY_T(tuple->time()), this));
    }
    else
    {
        // agent_->scheduleAt (simTime()+DELAY_T(MIN(tuple_->time(), tuple_->sym_time())),this);
        agent_->timerQueuePtr->insert(std::pair<simtime_t, OLSRPO_Timer *>(simTime()+DELAY_T(MIN(tuple->time(), tuple->sym_time())), this));
    }
}

OLSRPO_LinkTupleTimer::~OLSRPO_LinkTupleTimer()
{
    removeTimer();
    if (!tuple_)
        return;
...
void
OLSRPO_Nb2hopTupleTimer::expire()
{
    OLSRPO_nb2hop_tuple* tuple = dynamic_cast<OLSRPO_nb2hop_tuple*> (tuple_);
    double time = tuple->time();
    if (time < SIMTIME_DBL(simTime()))
    {
        removeTimer();
...
OLSRPO_Nb2hopTupleTimer::~OLSRPO_Nb2hopTupleTimer()
{
    removeTimer();
    if (!tuple_)
        return;
...
void
OLSRPO_MprSelTupleTimer::expire()
{
    OLSRPO_mprsel_tuple* tuple = dynamic_cast<OLSRPO_mprsel_tuple*> (tuple_);
    double time = tuple->time();
    if (time < SIMTIME_DBL(simTime()))
    {
        removeTimer();
...
OLSRPO_MprSelTupleTimer::~OLSRPO_MprSelTupleTimer()
{
    removeTimer();
    if (!tuple_)
        return;
...
void
OLSRPO_TopologyTupleTimer::expire()
{
    OLSRPO_topology_tuple* tuple = dynamic_cast<OLSRPO_topology_tuple*> (tuple_);
    double time = tuple->time();
    if (time < SIMTIME_DBL(simTime()))
    {
        removeTimer();
...
OLSRPO_TopologyTupleTimer::~OLSRPO_TopologyTupleTimer()
{
    removeTimer();
    if (!tuple_)
        return;
...
void OLSRPO_IfaceAssocTupleTimer::expire()
{
    OLSRPO_iface_assoc_tuple* tuple = dynamic_cast<OLSRPO_iface_assoc_tuple*> (tuple_);
    double time = tuple->time();
    if (time < SIMTIME_DBL(simTime()))
    {
        removeTimer();
...
OLSRPO_IfaceAssocTupleTimer::~OLSRPO_IfaceAssocTupleTimer()
{
    removeTimer();
    if (!tuple_)
        return;
...
///
/// \brief Sends a control packet which must bear every message in the OLSRPO agent's buffer.
///
/// The task of actually sending the packet is left to the OLSRPO agent.
///
/// \param e The event which has expired.
///
void
OLSRPO_MsgTimer::expire()
{
    agent_->send_pkt();
    removeTimer();
    delete this;
...
void OLSRPO::initialize(int stage)
{
	...
	///
	    OLSRPO_REFRESH_INTERVAL=par("OLSRPO_REFRESH_INTERVAL");
        ...
        pkt_seq_ = OLSRPO_MAX_SEQ_NUM;
        msg_seq_ = OLSRPO_MAX_SEQ_NUM;
        ansn_ = OLSRPO_MAX_SEQ_NUM;
        ...
        helloTimer = new OLSRPO_HelloTimer(); ///< Timer for sending HELLO messages.
        tcTimer = new OLSRPO_TcTimer();   ///< Timer for sending TC messages.
        midTimer = new OLSRPO_MidTimer(); ///< Timer for sending MID messages.

        state_ptr = new OLSRPO_state();
        ...
        for (int i = 0; i< getNumWlanInterfaces(); i++)
        {
            ...
            OLSRPO_iface_assoc_tuple* tuple = new OLSRPO_iface_assoc_tuple;
...
///
/// If it is an %OLSRPO packet then it is processed. In other case, if it is a data packet
...
void OLSRPO::handleMessage(cMessage *msg)
{
    ...
    {
        //OLSRPO_Timer *timer=dynamic_cast<OLSRPO_Timer*>(msg);
        while (timerQueuePtr->begin()->first<=simTime())
        {
            OLSRPO_Timer *timer = timerQueuePtr->begin()->second;
            if (timer==nullptr)
                throw cRuntimeError("timer ower is bad");
            else
            {
...
///
/// \brief Check if packet is OLSRPO
/// \param p received packet.
///

OLSRPO_pkt *
OLSRPO::check_packet(cPacket* msg, nsaddr_t &src_addr, int &index)
{
    ...
    if (isInMacLayer())
    {
        op = dynamic_cast<OLSRPO_pkt  *>(msg);
        ...
        else
        {
            ...
            inet::Ieee802Ctrl* ctrl = check_and_cast<inet::Ieee802Ctrl*>(msg->removeControlInfo());
            src_addr = inet::L3Address(ctrl->getSrc());
            delete ctrl;
            return dynamic_cast<OLSRPO_pkt  *>(msg);
        }
    ...
    if (dynamic_cast<UDPPacket *>(msg)) // Check is Udp packet
    {
        ...
        if (!dynamic_cast<OLSRPO_pkt  *>(msg_aux)) // Check if olsr packet
        {
            delete  msg;
            delete msg_aux;
            return nullptr;
...
// Extract information and delete the cantainer without more use
    op = check_and_cast<OLSRPO_pkt  *>(msg_aux);
    ...
    inet::inet::INetworkProtocolControlInfo* controlInfo = check_and_cast<inet::INetworkProtocolControlInfo*>(msg->removeControlInfo());
...
///
/// \brief Processes an incoming %OLSRPO packet following RFC 3626 specification.
/// \param p received packet.
///
void
OLSRPO::recv_olsr(cMessage* msg)
{
    ...
    if (op->getByteLength() < OLSRPO_PKT_HDR_SIZE + OLSRPO_MSG_HDR_SIZE)
    {
        delete op;
        return;
    }
...
// Process Olsr information
    ...
    for (int i = 0; i < (int) op->msgArraySize(); i++)
    {
        ...
        OLSRPO_dup_tuple* duplicated = state_.find_dup_tuple(msg.orig_addr(), msg.msg_seq_num());
        if (duplicated == nullptr)
        {
            // Process the message according to its type
            if (msg.msg_type() == OLSRPO_HELLO_MSG)
                process_hello(msg, receiverIfaceAddr, src_addr, index);
            else if (msg.msg_type() == OLSRPO_TC_MSG)
                process_tc(msg, src_addr, index);
            else if (msg.msg_type() == OLSRPO_MID_MSG)
                process_mid(msg, src_addr, index);
            else
            {
                debug("%f: Node %s can not process OLSRPO packet because does not "
                      "implement OLSRPO type (%x)\n",
                      CURRENT_TIME,
                      getNodeId(ra_addr()),
                      msg.msg_type());
            }
    ...
    // After processing all OLSRPO messages, we must recompute routing table
    rtable_computation();
}


void OLSRPO::CoverTwoHopNeighbors(const nsaddr_t &neighborMainAddr, nb2hopset_t & N2)
{ // first gather all 2-hop neighbors to be removed
    std::set<nsaddr_t> toRemove;
    for (auto it = N2.begin(); it != N2.end(); it++)
    {
        OLSRPO_nb2hop_tuple* twoHopNeigh = *it;
...
OLSRPO::mpr_computation()
{
    ...
    for (auto it = nbset().begin(); it != nbset().end(); it++)
        if ((*it)->getStatus() == OLSRPO_STATUS_SYM) // I think that we need this check
            N.push_back(*it);
    ...
    {
        OLSRPO_nb2hop_tuple* nb2hop_tuple = *it;
        bool ok = true;

        OLSRPO_nb_tuple* nb_tuple = state_.find_sym_nb_tuple(nb2hop_tuple->nb_main_addr());
        if (nb_tuple == nullptr)
            ok = false;
        else
        {
            nb_tuple = state_.find_nb_tuple(nb2hop_tuple->nb_main_addr(), OLSRPO_WILL_NEVER);
            if (nb_tuple != nullptr)
                ok = false;
            else
            {
    ...
    {
        OLSRPO_nb_tuple* nb_tuple = *it;
        if (nb_tuple->willingness() == OLSRPO_WILL_ALWAYS)
    ...
    {
        increment = true;
        OLSRPO_nb2hop_tuple* nb2hop_tuple1 = *it;
        ...
        {
            OLSRPO_nb2hop_tuple* nb2hop_tuple2 = *it2;
    ...
    while (N2.begin() != N2.end())
    {
        ...
        std::map<int, std::vector<OLSRPO_nb_tuple*> > reachability;
        std::set<int> rs;
        for (auto it = N.begin(); it != N.end(); it++)
        {
            OLSRPO_nb_tuple* nb_tuple = *it;
            ...
            {
                OLSRPO_nb2hop_tuple* nb2hop_tuple = *it2;
...
OLSRPO::mpr_computation()
{
    ...
    for (auto it = nb2hopset().begin(); it != nb2hopset().end(); it++)
    {
        ...
        {
            OLSRPO_nb_tuple* neigh = *it2;
            if (neigh->nb_main_addr() == nb2hop_tuple->nb_main_addr())
            {
                if (neigh->willingness() == OLSRPO_WILL_NEVER)
                {
                    ok = false;
                    break;
                }
    ...
    for (auto it = N2.begin(); it != N2.end(); it++)
    {
        ...
        {
            OLSRPO_nb2hop_tuple* otherTwoHopNeigh = *it2;
...
///
void
OLSRPO::rtable_computation()
{
    nsaddr_t netmask(inet::IPv4Address::ALLONES_ADDRESS);
    ...
    {
        OLSRPO_nb_tuple* nb_tuple = *it;
        if (nb_tuple->getStatus() == OLSRPO_STATUS_SYM)
        {
            bool nb_main_addr = false;
            OLSRPO_link_tuple* lt = nullptr;
            for (auto it2 = linkset().begin(); it2 != linkset().end(); it2++)
            {
                OLSRPO_link_tuple* link_tuple = *it2;
    ...
    for (auto it = nb2hopset().begin(); it != nb2hopset().end(); it++)
    {
        ...
        if (ok)
        {
            OLSRPO_rt_entry* entry = rtable_.lookup(nb2hop_tuple->nb_main_addr());
            assert(entry != nullptr);
            // check if the entry is already in the routing table and the new is a better alternative
            bool insert = true;
            OLSRPO_rt_entry* entry2hop = rtable_.lookup(nb2hop_tuple->nb2hop_addr());
            if (entry2hop)
            {
                // check if the node is a better alternative
                OLSRPO_nb_tuple* nb_tupleNew = state_.find_sym_nb_tuple(nb2hop_tuple->nb_main_addr());
                OLSRPO_nb_tuple* nb_tupleOld = state_.find_sym_nb_tuple(entry2hop->next_addr());
    ...
    for (uint32_t h = 2;; h++)
    {
        ...
        for (auto it = topologyset().begin();
                ...
                it++)
        {
            OLSRPO_topology_tuple* topology_tuple = *it;
            OLSRPO_rt_entry* entry1 = rtable_.lookup(topology_tuple->dest_addr());
            OLSRPO_rt_entry* entry2 = rtable_.lookup(topology_tuple->last_addr());
                ...
                it++)
        {
            ...
            OLSRPO_rt_entry* entry1 = rtable_.lookup(tuple->main_addr());
            OLSRPO_rt_entry* entry2 = rtable_.lookup(tuple->iface_addr());
...
///
/// \param msg the %OLSRPO message which contains the HELLO message.
/// \param receiver_iface the address of the interface where the message was received from.
/// \param sender_iface the address of the interface where the message was sent from.
///
bool
OLSRPO::process_hello(OLSRPO_msg& msg, const nsaddr_t &receiver_iface, const nsaddr_t &sender_iface, const int &index)
{
    assert(msg.msg_type() == OLSRPO_HELLO_MSG);
...
///
/// \param msg the %OLSRPO message which contains the TC message.
/// \param sender_iface the address of the interface where the message was sent from.
///
bool
OLSRPO::process_tc(OLSRPO_msg& msg, const nsaddr_t &sender_iface, const int &index)
{
    assert(msg.msg_type() == OLSRPO_TC_MSG);
    double now = CURRENT_TIME;
    OLSRPO_tc& tc = msg.tc();

    // 1. If the sender interface of this message is not in the symmetric
    // 1-hop neighborhood of this node, the message MUST be discarded.
    OLSRPO_link_tuple* link_tuple = state_.find_sym_link_tuple(sender_iface, now);
    ...
    // performed.
    OLSRPO_topology_tuple* topology_tuple =
    ...
    for (int i = 0; i < tc.count; i++)
    {
        ...
        if (topology_tuple != nullptr)
            topology_tuple->time() = now + OLSRPO::emf_to_seconds(msg.vtime());
        ...
        {
            OLSRPO_topology_tuple* topology_tuple = new OLSRPO_topology_tuple;
            ...
            OLSRPO_TopologyTupleTimer* topology_timer =
                new OLSRPO_TopologyTupleTimer(this, topology_tuple);
...
///
/// \param msg the %OLSRPO message which contains the MID message.
/// \param sender_iface the address of the interface where the message was sent from.
///
void
OLSRPO::process_mid(OLSRPO_msg& msg, const nsaddr_t &sender_iface, const int &index)
{
    assert(msg.msg_type() == OLSRPO_MID_MSG);
    double now = CURRENT_TIME;
    OLSRPO_mid& mid = msg.mid();
    ...
    for (int i = 0; i < mid.count; i++)
    {
                ...
                it++)
        {
            ...
            {
                tuple->time() = now + OLSRPO::emf_to_seconds(msg.vtime());
                updated = true;
            }
        }
        if (!updated)
        {
            ...
            OLSRPO_IfaceAssocTupleTimer* ifaceassoc_timer =
                new OLSRPO_IfaceAssocTupleTimer(this, tuple);
...
///
/// \brief OLSRPO's default forwarding algorithm.
///
/// See RFC 3626 for details.
///
/// \param p the %OLSRPO packet which has been received.
/// \param msg the %OLSRPO message which must be forwarded.
...
///
void
OLSRPO::forward_default(OLSRPO_msg& msg, OLSRPO_dup_tuple* dup_tuple, const nsaddr_t &local_iface, const nsaddr_t &src_addr)
{
    double now = CURRENT_TIME;
    ...
    OLSRPO_link_tuple* link_tuple = state_.find_sym_link_tuple(src_addr, now);
    if (link_tuple == nullptr)
        return;
    ...
    {
        dup_tuple->time() = now + OLSRPO_DUP_HOLD_TIME;
    ...
    {
        OLSRPO_dup_tuple* new_dup = new OLSRPO_dup_tuple;
        new_dup->getAddr() = msg.orig_addr();
        new_dup->seq_num() = msg.msg_seq_num();
        new_dup->time() = now + OLSRPO_DUP_HOLD_TIME;
        ...
        OLSRPO_DupTupleTimer* dup_timer =
            new OLSRPO_DupTupleTimer(this, new_dup);
...
#if 0
void
OLSRPO::forward_data(cMessage* p, nsaddr_t addr)
{
    ...
    else
    {
        ...
        {
            OLSRPO_rt_entry* entry = rtable_.lookup(ih->daddr());
            if (entry == nullptr)
            {
                debug("%f: Node %d can not forward a packet destined to %d\n",
                      CURRENT_TIME,
                      OLSRPO::node_id(ra_addr()),
                      OLSRPO::node_id(ih->daddr()));
...
#endif
///
/// \brief Enques an %OLSRPO message which will be sent with a delay of (0, delay].
///
/// This buffering system is used in order to piggyback several %OLSRPO messages in
/// a same %OLSRPO packet.
///
/// \param msg the %OLSRPO message which must be sent.
/// \param delay maximum delay the %OLSRPO message is going to be buffered.
///
void
OLSRPO::enque_msg(OLSRPO_msg& msg, double delay)
{
    assert(delay >= 0);

    msgs_.push_back(msg);
    OLSRPO_MsgTimer* timer = new OLSRPO_MsgTimer(this);
    timer->resched(delay);
}

///
/// \brief Creates as many %OLSRPO packets as needed in order to send all buffered
/// %OLSRPO messages.
///
/// Maximum number of messages which can be contained in an %OLSRPO packet is
/// dictated by OLSRPO_MAX_MSGS constant.
///
void
OLSRPO::send_pkt()
{
    ...
    int num_pkts = (num_msgs%OLSRPO_MAX_MSGS == 0) ? num_msgs/OLSRPO_MAX_MSGS :
                   (num_msgs/OLSRPO_MAX_MSGS + 1);

    inet::L3Address destAdd;
    if (!this->isInMacLayer())
        destAdd.set(inet::IPv4Address::ALLONES_ADDRESS);
    else
        destAdd.set(MACAddress::BROADCAST_ADDRESS);

    for (int i = 0; i < num_pkts; i++)
    {
        OLSRPO_pkt* op = new OLSRPO_pkt;
        op->setName("OLSRPO Pkt");

        op->setByteLength( OLSRPO_PKT_HDR_SIZE );
        ...
        sendToIp(op, RT_PORT, destAdd, RT_PORT, IP_DEF_TTL, 0.0, inet::L3Address());
    }
}

///
/// \brief Creates a new %OLSRPO HELLO message which is buffered to be sent later on.
///
void
OLSRPO::send_hello()
{
    OLSRPO_msg msg;
    double now = CURRENT_TIME;
    msg.msg_type() = OLSRPO_HELLO_MSG;
    msg.vtime() = OLSRPO::seconds_to_emf(OLSRPO_NEIGHB_HOLD_TIME);
    ...
    msg.hello().htime() = OLSRPO::seconds_to_emf(hello_ival());
    ...
    {
        OLSRPO_link_tuple* link_tuple = *it;
        if (get_main_addr(link_tuple->local_iface_addr()) == ra_addr() && link_tuple->time() >= now)
        {
            ...
            else
            {
                ...
                for (auto nb_it = nbset().begin();
                        ...
                        nb_it++)
                {
                    OLSRPO_nb_tuple* nb_tuple = *nb_it;
                    if (nb_tuple->nb_main_addr() == get_main_addr(link_tuple->nb_iface_addr()))
                    {
                        if (nb_tuple->getStatus() == OLSRPO_STATUS_SYM)
                            nb_type = OLSRPO_SYM_NEIGH;
                        else if (nb_tuple->getStatus() == OLSRPO_STATUS_NOT_SYM)
                            nb_type = OLSRPO_NOT_NEIGH;
                        else
                        {
            ...
            if (pos == linkcodes_count.end())
            {
                ...
                assert(count >= 0 && count < OLSRPO_MAX_HELLOS);
...
///
/// \brief Creates a new %OLSRPO TC message which is buffered to be sent later on.
///
void
OLSRPO::send_tc()
{
    OLSRPO_msg msg;
    msg.msg_type() = OLSRPO_TC_MSG;
    msg.vtime() = OLSRPO::seconds_to_emf(OLSRPO_TOP_HOLD_TIME);
    msg.orig_addr() = ra_addr();
    msg.ttl() = 255;
    msg.hop_count() = 0;
    msg.msg_seq_num() = msg_seq();
    ...
    {
        OLSRPO_mprsel_tuple* mprsel_tuple = *it;
        int count = msg.tc().count;

        assert(count >= 0 && count < OLSRPO_MAX_ADDRS);
...
///
/// \brief Creates a new %OLSRPO MID message which is buffered to be sent later on.
...
OLSRPO::send_mid()
{
    if (getNumWlanInterfaces() <= 1 && optimizedMid)
        return;
    OLSRPO_msg msg;
    msg.msg_type() = OLSRPO_MID_MSG;
    msg.vtime() = OLSRPO::seconds_to_emf(OLSRPO_MID_HOLD_TIME);
    msg.orig_addr() = ra_addr();
    msg.ttl() = 255;
    msg.hop_count() = 0;
    msg.msg_seq_num() = msg_seq();
...
///
bool
OLSRPO::link_sensing(OLSRPO_msg& msg, const nsaddr_t &receiver_iface, const nsaddr_t &sender_iface, const int &index)
{
    OLSRPO_hello& hello = msg.hello();
    double now = CURRENT_TIME;
    bool updated = false;
    bool created = false;

    OLSRPO_link_tuple* link_tuple = state_.find_link_tuple(sender_iface);
    if (link_tuple == nullptr)
    {
        // We have to create a new tuple
        link_tuple = new OLSRPO_link_tuple;
        ...
        link_tuple->time() = now + OLSRPO::emf_to_seconds(msg.vtime());
    ...
    link_tuple->asym_time() = now + OLSRPO::emf_to_seconds(msg.vtime());
    assert(hello.count >= 0 && hello.count <= OLSRPO_MAX_HELLOS);
    for (int i = 0; i < hello.count; i++)
    {
        OLSRPO_hello_msg& hello_msg = hello.hello_msg(i);
        int lt = hello_msg.link_code() & 0x03;
        int nt = hello_msg.link_code() >> 2;

        // We must not process invalid advertised links
        if ((lt == OLSRPO_SYM_LINK && nt == OLSRPO_NOT_NEIGH) ||
                (nt != OLSRPO_SYM_NEIGH && nt != OLSRPO_MPR_NEIGH
                 && nt != OLSRPO_NOT_NEIGH))
            continue;

        assert(hello_msg.count >= 0 && hello_msg.count <= OLSRPO_MAX_ADDRS);
        for (int j = 0; j < hello_msg.count; j++)
        {
            if (hello_msg.nb_iface_addr(j) == receiver_iface)
            {
                if (lt == OLSRPO_LOST_LINK)
                {
                    link_tuple->sym_time() = now - 1;
                    updated = true;
                }
                else if (lt == OLSRPO_SYM_LINK || lt == OLSRPO_ASYM_LINK)
                {
                    link_tuple->sym_time() =
                        now + OLSRPO::emf_to_seconds(msg.vtime());
                    link_tuple->time() =
                        link_tuple->sym_time() + OLSRPO_NEIGHB_HOLD_TIME;
                    link_tuple->lost_time() = 0.0;
                    updated = true;
                }
                break;
    ...
    {
        OLSRPO_LinkTupleTimer* link_timer =
            new OLSRPO_LinkTupleTimer(this, link_tuple);
...
///
bool
OLSRPO::populate_nbset(OLSRPO_msg& msg)
{
    OLSRPO_hello& hello = msg.hello();

    OLSRPO_nb_tuple* nb_tuple = state_.find_nb_tuple(msg.orig_addr());
    if (nb_tuple != nullptr)
...
///
bool
OLSRPO::populate_nb2hopset(OLSRPO_msg& msg)
{
    double now = CURRENT_TIME;
    OLSRPO_hello& hello = msg.hello();
    ...
    {
        OLSRPO_link_tuple* link_tuple = *it_lt;
            ...
            if (link_tuple->sym_time() >= now)
            {
                ...
                for (int i = 0; i < hello.count; i++)
                {
                    ...
                    assert(hello_msg.count >= 0 &&
                           hello_msg.count <= OLSRPO_MAX_ADDRS);

                    for (int j = 0; j < hello_msg.count; j++)
                    {
                        nsaddr_t nb2hop_addr = get_main_addr(hello_msg.nb_iface_addr(j));
                        if (nt == OLSRPO_SYM_NEIGH || nt == OLSRPO_MPR_NEIGH)
                        {
                            ...
                            if (nb2hop_addr != ra_addr())
                            {
                                ...
                                if (nb2hop_tuple == nullptr)
                                {
                                    ...
                                    // deletion
                                    OLSRPO_Nb2hopTupleTimer* nb2hop_timer =
                                        new OLSRPO_Nb2hopTupleTimer(this, nb2hop_tuple);
...
///
void
OLSRPO::populate_mprselset(OLSRPO_msg& msg)
{
    double now = CURRENT_TIME;
    OLSRPO_hello& hello = msg.hello();
        ...
        if (nt == OLSRPO_MPR_NEIGH)
        {
            ...
            for (int j = 0; j < hello_msg.count; j++)
            {
                if (get_main_addr(hello_msg.nb_iface_addr(j)) == ra_addr())
                {
                    ...
                    if (mprsel_tuple == nullptr)
                    {
                        mprsel_tuple = new OLSRPO_mprsel_tuple;
                        mprsel_tuple->main_addr() = msg.orig_addr();
                        mprsel_tuple->time() =
                            now + OLSRPO::emf_to_seconds(msg.vtime());
                        add_mprsel_tuple(mprsel_tuple);
                        // Schedules mpr selector tuple deletion
                        OLSRPO_MprSelTupleTimer* mprsel_timer =
                            new OLSRPO_MprSelTupleTimer(this, mprsel_tuple);
...
///
void
OLSRPO::mac_failed(inet::IPv4Datagram* p)
{
    double now = CURRENT_TIME;
    ...
    EV_WARN <<"Node " << OLSRPO::node_id(ra_addr()) << "MAC Layer detects a breakage on link to "  <<
    OLSRPO::node_id(dest_addr);

    if (dest_addr == inet::L3Address(inet::IPv4Address(IP_BROADCAST)))
    {
        return;
    }

    OLSRPO_rt_entry*  entry = rtable_.lookup(dest_addr);
    if (entry)
    {
        OLSRPO_link_tuple* link_tuple = state_.find_link_tuple(entry->next_addr());
        if (link_tuple != nullptr)
        {
            link_tuple->lost_time() = now + OLSRPO_NEIGHB_HOLD_TIME;
            link_tuple->time() = now + OLSRPO_NEIGHB_HOLD_TIME;
            nb_loss(link_tuple);
        }
    }
...
///
void
OLSRPO::nb_loss(OLSRPO_link_tuple* tuple)
{
    ...
    updated_link_tuple(tuple,OLSRPO_WILL_DEFAULT);
...
///
void
OLSRPO::add_dup_tuple(OLSRPO_dup_tuple* tuple)
{
    /*debug("%f: Node %d adds dup tuple: addr = %d seq_num = %d\n",
        CURRENT_TIME,
        OLSRPO::node_id(ra_addr()),
        OLSRPO::node_id(tuple->getAddr()),
        tuple->seq_num());*/
...
///
void
OLSRPO::rm_dup_tuple(OLSRPO_dup_tuple* tuple)
{
...
///
void
OLSRPO::add_link_tuple(OLSRPO_link_tuple* tuple, uint8_t  willingness)
{
    double now = CURRENT_TIME;
    ...
    OLSRPO_nb_tuple* nb_tuple = new OLSRPO_nb_tuple;
    nb_tuple->nb_main_addr() = get_main_addr(tuple->nb_iface_addr());
    nb_tuple->willingness() = willingness;
    if (tuple->sym_time() >= now)
        nb_tuple->getStatus() = OLSRPO_STATUS_SYM;
    else
        nb_tuple->getStatus() = OLSRPO_STATUS_NOT_SYM;
...
///
void
OLSRPO::rm_link_tuple(OLSRPO_link_tuple* tuple)
{
    ...
    OLSRPO_nb_tuple* nb_tuple = state_.find_nb_tuple(nb_addr);
...
///
void
OLSRPO::updated_link_tuple(OLSRPO_link_tuple* tuple, uint8_t willingness)
{
    double now = CURRENT_TIME;

    // Each time a link tuple changes, the associated neighbor tuple must be recomputed
    OLSRPO_nb_tuple* nb_tuple = find_or_add_nb(tuple, willingness);
    ...
    if (nb_tuple != nullptr)
    {
        ...
        debug("%f: Node %s has updated link tuple: nb_addr = %s status = %s\n", now, getNodeId(ra_addr()),
                getNodeId(tuple->nb_iface_addr()), ((nb_tuple->getStatus() == OLSRPO_STATUS_SYM) ? "sym" : "not_sym"));
    }
}
// Auxiliary method
// add NB based in the link tuple information if the Nb doen't exist

OLSRPO_nb_tuple* OLSRPO::find_or_add_nb(OLSRPO_link_tuple* tuple, uint8_t willingness)
{
    OLSRPO_nb_tuple* nb_tuple = state_.find_nb_tuple(get_main_addr(tuple->nb_iface_addr()));
    if (nb_tuple == nullptr)
    {
        double now = CURRENT_TIME;
        state_.erase_nb_tuple(tuple->nb_iface_addr());
        // Creates associated neighbor tuple
        nb_tuple = new OLSRPO_nb_tuple;
        nb_tuple->nb_main_addr() = get_main_addr(tuple->nb_iface_addr());
        nb_tuple->willingness() = willingness;
        if (tuple->sym_time() >= now)
            nb_tuple->getStatus() = OLSRPO_STATUS_SYM;
...
///
void
OLSRPO::add_nb_tuple(OLSRPO_nb_tuple* tuple)
{
    debug("%f: Node %s adds neighbor tuple: nb_addr = %s status = %s\n",
          CURRENT_TIME,
          getNodeId(ra_addr()),
          getNodeId(tuple->nb_main_addr()),
          ((tuple->getStatus() == OLSRPO_STATUS_SYM) ? "sym" : "not_sym"));
...
///
void
OLSRPO::rm_nb_tuple(OLSRPO_nb_tuple* tuple)
{
...
///
void
OLSRPO::add_nb2hop_tuple(OLSRPO_nb2hop_tuple* tuple)
{
...
///
void
OLSRPO::rm_nb2hop_tuple(OLSRPO_nb2hop_tuple* tuple)
{
...
///
void
OLSRPO::add_mprsel_tuple(OLSRPO_mprsel_tuple* tuple)
{
    ...
    ansn_ = (ansn_ + 1)%(OLSRPO_MAX_SEQ_NUM + 1);
}
...
///
void
OLSRPO::rm_mprsel_tuple(OLSRPO_mprsel_tuple* tuple)
{
...
///
void
OLSRPO::add_topology_tuple(OLSRPO_topology_tuple* tuple)
{
...
///
void
OLSRPO::rm_topology_tuple(OLSRPO_topology_tuple* tuple)
{
...
///
void
OLSRPO::add_ifaceassoc_tuple(OLSRPO_iface_assoc_tuple* tuple)
{
...
///
void
OLSRPO::rm_ifaceassoc_tuple(OLSRPO_iface_assoc_tuple* tuple)
{
...
///
const nsaddr_t &
OLSRPO::get_main_addr(const nsaddr_t &iface_addr) const
{
    OLSRPO_iface_assoc_tuple* tuple =
...
///
bool
OLSRPO::seq_num_bigger_than(uint16_t s1, uint16_t s2)
{
    return (s1 > s2 && s1-s2 <= OLSRPO_MAX_SEQ_NUM/2)
           || (s2 > s1 && s2-s1 > OLSRPO_MAX_SEQ_NUM/2);
}
...
///
int
OLSRPO::degree(OLSRPO_nb_tuple* tuple)
{
    int degree = 0;
    for (auto it = nb2hopset().begin(); it != nb2hopset().end(); it++)
    {
        ...
        {
            //OLSRPO_nb_tuple* nb_tuple =
            //    state_.find_nb_tuple(nb2hop_tuple->nb_main_addr());
            OLSRPO_nb_tuple* nb_tuple = state_.find_nb_tuple(nb2hop_tuple->nb2hop_addr());
            if (nb_tuple == nullptr)
                degree++;
        }
    }
...
///
uint8_t
OLSRPO::seconds_to_emf(double seconds)
{
    // This implementation has been taken from unik-olsrd-0.4.5 (mantissa.c),
    // licensed under the GNU Public License (GPL)

    int a, b = 0;
    while (seconds/OLSRPO_C >= pow((double)2, (double)b))
        b++;
    b--;
    ...
    {
        a = (int)(16*((double)seconds/(OLSRPO_C*(double)pow((double)2, b))-1));
        while (a >= 16)
        {
            a -= 16;
            b++;
...
///
double
OLSRPO::emf_to_seconds(uint8_t olsr_format)
{
    // This implementation has been taken from unik-olsrd-0.4.5 (mantissa.c),
    // licensed under the GNU Public License (GPL)
    int a = olsr_format >> 4;
    int b = olsr_format - a*16;
    return (double)(OLSRPO_C*(1+(double)a/16)*(double)pow((double)2, b));
}

///
/// \brief Returns the identifier of a node given the address of the attached OLSRPO agent.
///
/// \param addr the address of the OLSRPO routing agent.
/// \return the identifier of the node.
///
int
OLSRPO::node_id(const nsaddr_t &addr)
{
...
const char * OLSRPO::getNodeId(const nsaddr_t &addr)
{
    return addr.str().c_str();
}

// Interfaces with other Inet
void OLSRPO:: processLinkBreak(const cObject *details)
{
    if (use_mac())
    {
        inet::IPv4Datagram * dgram = dynamic_cast<inet::IPv4Datagram *>(const_cast<cObject*>(details));
        if (dgram)
            mac_failed(dgram);
    }
}
...
uint32_t OLSRPO::getRoute(const inet::L3Address &dest, std::vector<inet::L3Address> &add)
{
    add.clear();
    OLSRPO_rt_entry* rt_entry = rtable_.lookup(dest);
    inet::L3Address apAddr;
    if (!rt_entry)
    {
        if (getAp(dest, apAddr))
        {
            OLSRPO_rt_entry* rt_entry = rtable_.lookup(apAddr);
            if (!rt_entry)
                return 0;
            for (int i = 0; i < (int) rt_entry->route.size(); i++)
                add.push_back(rt_entry->route[i]);
            add.push_back(apAddr);
            OLSRPO_rt_entry* rt_entry_aux = rtable_.find_send_entry(rt_entry);
            if (rt_entry_aux->next_addr() != add[0])
                throw cRuntimeError("OLSRPO Data base error");
...
bool OLSRPO::getNextHop(const inet::L3Address &dest, inet::L3Address &add, int &iface, double &cost)
{
    OLSRPO_rt_entry* rt_entry = rtable_.lookup(dest);
    if (!rt_entry)
    {
...
bool OLSRPO::isProactive()
{
    return true;
}


bool OLSRPO::isOurType(cPacket * msg)
{
    OLSRPO_pkt * pkt = dynamic_cast<OLSRPO_pkt  *>(msg);
    if (pkt)
        return true;
    return false;
}

bool OLSRPO::getDestAddress(cPacket *msg, inet::L3Address &dest)
{
    return false;
}

void OLSRPO::scheduleNextEvent()
{
    ...
    if (timerMessage->isScheduled())
    {
        if (e->first < timerMessage->getArrivalTime())
        {
            ...
            throw cRuntimeError("OLSRPO timer Queue problem");
    }
    else
    {
        scheduleAt(e->first, timerMessage);
...
int OLSRPO::getRouteGroup(const inet::inetmanet::AddressGroup &gr, std::vector<inet::L3Address> &add)
{

    int distance = 1000;
    add.clear();
    for (inet::inetmanet::AddressGroupConstIterator it = gr.begin(); it!=gr.end(); it++)
    {
        inet::L3Address dest = *it;
        OLSRPO_rt_entry* rt_entry = rtable_.lookup(dest);
        if (!rt_entry)
            continue;
...
bool OLSRPO::getNextHopGroup(const inet::inetmanet::AddressGroup &gr, inet::L3Address &add, int &iface, inet::L3Address &gw)
{
...
int  OLSRPO::getRouteGroup(const inet::L3Address& dest, std::vector<inet::L3Address> &add, inet::L3Address& gateway, bool &isGroup, int group)
{
    inet::inetmanet::AddressGroup gr;
    int distance = 0;
...
bool OLSRPO::getNextHopGroup(const inet::L3Address& dest, inet::L3Address &next, int &iface, inet::L3Address& gw, bool &isGroup, int group)
{
    inet::inetmanet::AddressGroup gr;
    bool find = false;
    if (findInAddressGroup(dest, group))
...
inet::L3Address OLSRPO::getIfaceAddressFromIndex(int index)
{
    InterfaceEntry * entry = getInterfaceEntry(index);
    if (this->isInMacLayer())
        return inet::L3Address(entry->getMacAddress());
    else
        return inet::L3Address(entry->ipv4Data()->getIPAddress());
}

void OLSRPO::computeDistributionPath(const nsaddr_t &initNode)
{
    ...
    while(!mpr.empty())
    {
        ...
        if (it==globalRtable.end())
            return;
        OLSRPO_rtable *val = it->second;
        auto itMin = mpr.end();
        int hops = 1000;
        OLSRPO_rt_entry* segmentRoute = nullptr;
        for (auto it2 = mpr.begin();it2 != mpr.end();++it2)
        {
            ...
            {
                OLSRPO_rt_entry*  entry = val->lookup(*it2);
                if (entry == nullptr)
                    return;
...
void OLSRPO::getDistributionPath(const inet::L3Address &addr,std::vector<inet::L3Address> &path)
{
...
bool
OLSRPO::isNodeCandidate(const nsaddr_t &src_addr)
{
    double now = CURRENT_TIME;
    ...
    OLSRPO_mprsel_tuple* mprsel_tuple = state_.find_mprsel_tuple(get_main_addr(src_addr));
    if (mprsel_tuple != nullptr)
        return true;
    return false;
}

bool OLSRPO::handleNodeStart(inet::IDoneCallback *doneCallback)
{
    hello_timer_.resched(hello_ival());
    tc_timer_.resched(hello_ival());
    mid_timer_.resched(hello_ival());
...
bool OLSRPO::handleNodeShutdown(inet::IDoneCallback *doneCallback)
{
