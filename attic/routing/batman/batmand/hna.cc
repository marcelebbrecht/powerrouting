// PowerRouting for OMNeT++ - Batman hna
// Marcel Ebbrecht, marcel.ebbrecht@googlemail.com
// free software, see LICENSE.md for details
// derived from inetmanet-3.5, hna.cc
...
unsigned char *hna_buff_local = nullptr;
...
static struct hashtable_t *hna_global_hash = nullptr;
...
void hna_init(void)
{
    ...
    pthread_mutex_init(&hna_chg_list_mutex, nullptr);

    /* hna global */
    hna_global_hash = hash_new(128, compare_hna, choose_hna);

    if (hna_global_hash == nullptr) {
...
void Batman::hna_local_task_add_ip(const L3Address &ip_addr, uint16_t netmask, uint8_t route_action)
{
    HnaTask hna_task;
...
void hna_local_task_add_str(char *hna_string, uint8_t route_action, uint8_t runtime)
{
    ...
    if ((slash_ptr = strchr(hna_string, '/')) == nullptr) {
        if (runtime) {
    ...
    *slash_ptr = '\0';
    ...
    errno = 0;
    netmask = strtol(slash_ptr + 1, nullptr, 10);
    ...
    while (!hna_chg_list.empty()) {
        ...
        for (unsigned int hna_pos = 0; hna_pos < hna_list.size(); hna_pos++) {
            ...
            if ((hna_task->addr == hna_local_entry->addr) && (hna_task->netmask == hna_local_entry->netmask)) {
                ...
                if (hna_task->route_action == ROUTE_DEL) {
                    EV_INFO << "Deleting HNA from announce network list: " << hna_task->addr << "/" << hna_task->netmask << endl;

                    hna_local_update_routes(hna_local_entry, ROUTE_DEL);
                    hna_list.erase(hna_list.begin()+hna_pos);
                    --hna_pos;
                } else {
                    EV_INFO << "Can't add HNA - already announcing network: " << hna_task->addr << "/" << hna_task->netmask << endl;
                }
                break;
            }
        }
        ...
        if (!found) {
            if (hna_task->route_action == ROUTE_ADD) {
                EV_INFO << "Adding HNA to announce network list: " << hna_task->addr << "/" << hna_task->netmask << endl;
                ...
                EV_INFO << "Can't delete HNA - network is not announced: " << hna_task->addr << "/" << hna_task->netmask << endl;
            }
        }
...
void Batman::hna_local_update_routes(HnaLocalEntry *hna_local_entry, int8_t route_action)
{
    ...
    add_del_route(hna_local_entry->addr, hna_local_entry->netmask, L3Address(), 0, nullptr, BATMAN_RT_TABLE_TUNNEL, ROUTE_TYPE_THROW, route_action);
...
void Batman::_hna_global_add(OrigNode *orig_node, HnaElement *hna_element)
{
    ...
    if (it == hnaMap.end())
    {
        ...
        hna_global_entry->curr_orig_node = nullptr;
...
void Batman::hna_global_add(OrigNode *orig_node, HnaElement *new_hna, int16_t new_hna_len)
{
    ...
    if ((new_hna == nullptr) || (new_hna_len == 0)) {
        return;
    }
    ...
    EV_INFO << "HNA information received (" << num_elements << " HNA network" << (num_elements > 1 ? "s": "") << "):\n";
    ...
    for (i = 0; i < num_elements; i++) {
        ...
        if ((e->netmask > 0) && (e->netmask < 33))
            EV_DETAIL << "hna: " << e->addr << "/" << (unsigned int)(e->netmask) << "\n";
        else
            EV_DETAIL << "hna: " << e->addr << "/" << (unsigned int)(e->netmask) << " -> ignoring (invalid netmask) \n";
...
void Batman::hna_global_update(OrigNode *orig_node, HnaElement *new_hna, int16_t new_hna_len, NeighNode *old_router)
{
    ...
    if ((!orig_node->hna_buff.empty()) && ((new_hna == nullptr) || (new_hna_len == 0))) {
     ...
     * NOTE: no nullptr pointer checking here because memcmp() just returns if n == 0
     */
    bool change = false;
...
static void _hna_global_hash_del(void *data)
{
    ...
    struct hna_orig_ptr *hna_orig_ptr = nullptr;
