...
// routing module, derived from AODVRouting.cc, version 0.2
...
//        host->subscribe(inet::NF_LINK_BREAK, this);
//
//        if (useHelloMessages) {
...
//
//// start modification
//// function for power calculation, if energyStorage kind of IdealEnergyStorage (when nominal capacity is infinite), return 1, else calculate penalty
//int AODVPORouting::calculatePenalty() {
//    if ( strcmp(host->getSubmodule("energyStorage")->getClassName(), "inet::power::SimpleEnergyStorage") ) {
//        EV_INFO << "Power Routing - StorageType is not SimpleEnergyStorage, returning one (normal behavior)" << endl;
//        return 1;
//    } else {
//        EV_INFO << "Power Routing - StorageType is SimpleEnergyStorage, calculating hop penalty according to energy-capacity" << endl;
//        relativeCharge = energyStorage->getResidualEnergyCapacity().get() / energyStorage->getNominalEnergyCapacity().get();
//        hopPenaltyDouble = ((1 / (relativeCharge / powerSensitivity))) + powerBias;
//        hopPenaltyDoublePreroundup = 0.999999999999999 + hopPenaltyDouble;
//        hopPenalty = (int)(hopPenaltyDoublePreroundup);
...
//        return 1;
//    } else {
//        EV_INFO << "Power Routing - StorageType is SimpleEnergyStorage, calculating trigger according to energy-capacity" << endl;
//        relativeCharge = energyStorage->getResidualEnergyCapacity().get() / energyStorage->getNominalEnergyCapacity().get();
//        powerTriggerDouble = relativeCharge / powerTrigger;
//        powerTriggerCalculated = (int)(powerTriggerDouble);
//        EV_INFO << "Power Routing - Capacities: " << energyStorage->getResidualEnergyCapacity() << " (actual) of " << energyStorage->getNominalEnergyCapacity() << " (nominal) " << endl;
...
//
//    if (msg->isSelfMessage()) {
//        if (dynamic_cast<inet::WaitForRREP *>(msg))
//            handleWaitForRREP((inet::WaitForRREP *)msg);
...
//    }
//    else if (inet::ICMPMessage *icmpPacket = dynamic_cast<inet::ICMPMessage *>(msg)) {
//        // ICMP packet arrived, dropped
//        delete icmpPacket;
//    }
//    else {
//        inet::UDPPacket *udpPacket = check_and_cast<inet::UDPPacket *>(msg);
//        inet::AODVControlPacket *ctrlPacket = check_and_cast<inet::AODVControlPacket *>(udpPacket->decapsulate());
//        inet::INetworkProtocolControlInfo *udpProtocolCtrlInfo = check_and_cast<inet::INetworkProtocolControlInfo *>(udpPacket->getControlInfo());
//        inet::L3Address sourceAddr = udpProtocolCtrlInfo->getSourceAddress();
//        unsigned int arrivalPacketTTL = udpProtocolCtrlInfo->getHopLimit();
//
//        switch (ctrlPacket->getPacketType()) {
//            case inet::RREQ:
//                handleRREQ(check_and_cast<inet::AODVRREQ *>(ctrlPacket), sourceAddr, arrivalPacketTTL);
//                break;
//
//            case inet::RREP:
//                handleRREP(check_and_cast<inet::AODVRREP *>(ctrlPacket), sourceAddr);
//                break;
//
//            case inet::RERR:
//                handleRERR(check_and_cast<inet::AODVRERR *>(ctrlPacket), sourceAddr);
//                break;
//
//            case inet::RREPACK:
//                handleRREPACK(check_and_cast<inet::AODVRREPACK *>(ctrlPacket), sourceAddr);
//                break;
//
//            default:
...
//}
//
//inet::INetfilter::IHook::Result AODVPORouting::ensureRouteForDatagram(inet::INetworkDatagram *datagram)
//{
...
//        inet::IRoute *route = routingTable->findBestMatchingRoute(destAddr);
//        inet::AODVRouteData *routeData = route ? dynamic_cast<inet::AODVRouteData *>(route->getProtocolData()) : nullptr;
...
//}
//
//AODVPORouting::AODVPORouting()
//{
//}
//
//bool AODVPORouting::hasOngoingRouteDiscovery(const inet::L3Address& target)
//{
//    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
//}
//
//void AODVPORouting::startRouteDiscovery(const inet::L3Address& target, unsigned timeToLive)
//{
...
//    inet::AODVRREQ *rreq = createRREQ(target);
//    addressToRreqRetries[target] = 0;
//    sendRREQ(rreq, addressType->getBroadcastAddress(), timeToLive);
//}
//
//inet::L3Address AODVPORouting::getSelfIPAddress() const
//{
//    return routingTable->getRouterIdAsGeneric();
//}
//
//void AODVPORouting::delayDatagram(inet::INetworkDatagram *datagram)
//{
//    EV_DETAIL << "Queuing datagram, source " << datagram->getSourceAddress() << ", destination " << datagram->getDestinationAddress() << endl;
//    const inet::L3Address& target = datagram->getDestinationAddress();
//    targetAddressToDelayedPackets.insert(std::pair<inet::L3Address, inet::INetworkDatagram *>(target, datagram));
//}
//
//void AODVPORouting::sendRREQ(inet::AODVRREQ *rreq, const inet::L3Address& destAddr, unsigned int timeToLive)
//{
...
//    inet::WaitForRREP *rrepTimerMsg = nullptr;
...
//    }
//    else {
//        rrepTimerMsg = new inet::WaitForRREP();
...
//}
//
//void AODVPORouting::sendRREP(inet::AODVRREP *rrep, const inet::L3Address& destAddr, unsigned int timeToLive)
//{
...
//
//    inet::IRoute *destRoute = routingTable->findBestMatchingRoute(destAddr);
//    const inet::L3Address& nextHop = destRoute->getNextHopAsGeneric();
//    inet::AODVRouteData *destRouteData = check_and_cast<inet::AODVRouteData *>(destRoute->getProtocolData());
...
//}
//
//inet::AODVRREQ *AODVPORouting::createRREQ(const inet::L3Address& destAddr)
//{
//    inet::AODVRREQ *rreqPacket = new inet::AODVRREQ("AODV-RREQ");
//
//    rreqPacket->setGratuitousRREPFlag(askGratuitousRREP);
//    inet::IRoute *lastKnownRoute = routingTable->findBestMatchingRoute(destAddr);
//
//    rreqPacket->setPacketType(inet::RREQ);
//
...
//}
//
//inet::AODVRREP *AODVPORouting::createRREP(inet::AODVRREQ *rreq, inet::IRoute *destRoute, inet::IRoute *originatorRoute, const inet::L3Address& lastHopAddr)
//{
//    inet::AODVRREP *rrep = new inet::AODVRREP("AODV-RREP");
...
//}
//
//inet::AODVRREP *AODVPORouting::createGratuitousRREP(inet::AODVRREQ *rreq, inet::IRoute *originatorRoute)
//{
//    ASSERT(originatorRoute != nullptr);
//    inet::AODVRREP *grrep = new inet::AODVRREP("AODV-GRREP");
...
//    // start modification
//    // add calculated penalty if its a non ideal energy storage, else one
//    unsigned int newHopCount = rrep->getHopCount() + this->calculatePenalty();
//    EV_INFO << "Power Routing - New Hop Count: " << rrep->getHopCount() + this->calculatePenalty() << endl;
//    // end modification
//    rrep->setHopCount(newHopCount);
...
//}
//
//void AODVPORouting::updateRoutingTable(inet::IRoute *route, const inet::L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
//{
...
//}
//
//void AODVPORouting::sendAODVPacket(AODVControlPacket *packet, const inet::L3Address& destAddr, unsigned int timeToLive, double delay)
//{
//    ASSERT(timeToLive != 0);
//
...
//}
//
//void AODVPORouting::handleRREQ(inet::AODVRREQ *rreq, const inet::L3Address& sourceAddr, unsigned int timeToLive)
//{
...
//
//    inet::IRoute *reverseRoute = routingTable->findBestMatchingRoute(rreq->getOriginatorAddr());
//
...
//
//    inet::IRoute *destRoute = routingTable->findBestMatchingRoute(rreq->getDestAddr());
...
//
//        // create RREP
//        inet::AODVRREP *rrep = createRREP(rreq, destRoute, reverseRoute, sourceAddr);
//
...
//
//            inet::IRoute *originatorRoute = routingTable->findBestMatchingRoute(rreq->getOriginatorAddr());
//            inet::AODVRREP *grrep = createGratuitousRREP(rreq, originatorRoute);
...
//
//        inet::AODVRREQ *outgoingRREQ = rreq->dup();
...
//}
//
//inet::IRoute *AODVPORouting::createRoute(const inet::L3Address& destAddr, const inet::L3Address& nextHop,
//        unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum,
//        bool isActive, simtime_t lifeTime)
//{
//    inet::IRoute *newRoute = routingTable->createRoute();
...
//    newRoute->setSourceType(inet::IRoute::AODV);
...
//}
//
//void AODVPORouting::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
//{
...
//        if (datagram) {
//            inet::L3Address unreachableAddr = datagram->getDestinationAddress();
...
//
//                inet::IRoute *route = routingTable->findBestMatchingRoute(unreachableAddr);
//
...
//}
//
//void AODVPORouting::handleLinkBreakSendRERR(const inet::L3Address& unreachableAddr)
//{
...
//
//    inet::IRoute *unreachableRoute = routingTable->findBestMatchingRoute(unreachableAddr);
//
...
//        inet::IRoute *route = routingTable->getRoute(i);
//
...
//}
//
//AODVRERR *AODVPORouting::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
//{
...
//}
//
//void AODVPORouting::handleRERR(AODVRERR *rerr, const inet::L3Address& sourceAddr)
//{
...
//}
//
//bool AODVPORouting::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
//{
//    Enter_Method_Silent();
...
//}
//
//void AODVPORouting::handleWaitForRREP(WaitForRREP *rrepTimer)
//{
//    EV_INFO << "We didn't get any Route Reply within RREP timeout" << endl;
//    inet::L3Address destAddr = rrepTimer->getDestAddr();
//
...
//    }
//
//    inet::AODVRREQ *rreq = createRREQ(destAddr);
//
...
//}
//
//void AODVPORouting::forwardRREP(inet::AODVRREP *rrep, const inet::L3Address& destAddr, unsigned int timeToLive)
//{
...
//}
//
//void AODVPORouting::forwardRREQ(inet::AODVRREQ *rreq, unsigned int timeToLive)
//{
...
//}
//
//void AODVPORouting::completeRouteDiscovery(const inet::L3Address& target)
//{
...
//}
//
//void AODVPORouting::sendGRREP(inet::AODVRREP *grrep, const inet::L3Address& destAddr, unsigned int timeToLive)
//{
...
//}
//
//inet::AODVRREP *AODVPORouting::createHelloMessage()
//{
...
//
//    inet::AODVRREP *helloMessage = new inet::AODVRREP("AODV-HelloMsg");
...
//}
//
//void AODVPORouting::sendHelloMessagesIfNeeded()
//{
...
//        inet::AODVRREP *helloMessage = createHelloMessage();
...
//}
//
//void AODVPORouting::handleHelloMessage(inet::AODVRREP *helloMessage)
//{
//    const inet::L3Address& helloOriginatorAddr = helloMessage->getDestAddr();
//    inet::IRoute *routeHelloOriginator = routingTable->findBestMatchingRoute(helloOriginatorAddr);
//
...
//}
//
//void AODVPORouting::expungeRoutes()
//{
//    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
//        inet::IRoute *route = routingTable->getRoute(i);
//        if (route->getSource() == this) {
...
//}
//
//void AODVPORouting::scheduleExpungeRoutes()
//{
...
//}
//
//INetfilter::IHook::Result AODVPORouting::datagramForwardHook(INetworkDatagram *datagram, const InterfaceEntry *inputInterfaceEntry, const InterfaceEntry *& outputInterfaceEntry, inet::L3Address& nextHopAddress)
//{
...
//    if ( powerTriggerLast > this->calculateTrigger() ) {
//        EV_INFO << "Power Routing - Capacity changed significantly, sending RERR to rebuild routes" << endl;
//        //updateValidRouteLifeTime(sourceAddr, simTime());
//        //updateValidRouteLifeTime(destAddr, simTime());
//        sendRERRWhenNoRouteToForward(sourceAddr);
//        sendRERRWhenNoRouteToForward(destAddr);
//    }
//    EV_INFO << "Power Routing - Updating power trigger, old: " << powerTriggerLast << ", new: " << powerTrigger << endl;
...
//
//    inet::IRoute *routeDest = routingTable->findBestMatchingRoute(destAddr);
...
//}
//
//void AODVPORouting::sendRERRWhenNoRouteToForward(const inet::L3Address& unreachableAddr)
//{
//    if (rerrCount >= rerrRatelimit) {
//        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
//        return;
...
//}
//
//void AODVPORouting::cancelRouteDiscovery(const inet::L3Address& destAddr)
//{
...
//}
//
//bool AODVPORouting::updateValidRouteLifeTime(const inet::L3Address& destAddr, simtime_t lifetime)
//{
//    inet::IRoute *route = routingTable->findBestMatchingRoute(destAddr);
//    if (route && route->getSource() == this) {
//        AODVRouteData *routeData = check_and_cast<AODVRouteData *>(route->getProtocolData());
...
//}
//
//inet::AODVRREPACK *AODVPORouting::createRREPACK()
//{
//    inet::AODVRREPACK *rrepACK = new inet::AODVRREPACK("AODV-RREPACK");
//    rrepACK->setPacketType(RREPACK);
//    rrepACK->setByteLength(2);
//    return rrepACK;
//}
//
//void AODVPORouting::sendRREPACK(inet::AODVRREPACK *rrepACK, const inet::L3Address& destAddr)
//{
//    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
//    sendAODVPacket(rrepACK, destAddr, 100, 0);
//}
//
//void AODVPORouting::handleRREPACK(inet::AODVRREPACK *rrepACK, const inet::L3Address& neighborAddr)
//{
...
//
//        inet::IRoute *route = routingTable->findBestMatchingRoute(neighborAddr);
//        if (route && route->getSource() == this) {
...
//}
//
//void AODVPORouting::handleRREPACKTimer()
//{
//    // when a node detects that its transmission of a RREP message has failed,
//    // it remembers the next-hop of the failed RREP in a "blacklist" set.
//
...
//}
//
//void AODVPORouting::handleBlackListTimer()
//{
