...
uint32 OlsrpoAddressSize::ADDR_SIZE = ADDR_SIZE_DEFAULT;
...
void OLSRPO::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_ROUTING_PROTOCOLS)
    {
       ...
       OlsrpoAddressSize::ADDR_SIZE = this->getAddressSize();
	///
...
///
void
OLSRPO::mac_failed(IPv4Datagram* p)
{
    double now = CURRENT_TIME;
...
uint32_t OLSRPO::getRoute(const L3Address &dest, std::vector<L3Address> &add)
{
    add.clear();
...
bool OLSRPO::getNextHop(const L3Address &dest, L3Address &add, int &iface, double &cost)
{
    OLSRPO_rt_entry* rt_entry = rtable_.lookup(dest);
    if (!rt_entry)
    {
...
bool OLSRPO::getDestAddress(cPacket *msg, L3Address &dest)
{
    return false;
}
...
int OLSRPO::getRouteGroup(const AddressGroup &gr, std::vector<L3Address> &add)
{
...
bool OLSRPO::getNextHopGroup(const AddressGroup &gr, L3Address &add, int &iface, L3Address &gw)
{
...
int  OLSRPO::getRouteGroup(const L3Address& dest, std::vector<L3Address> &add, L3Address& gateway, bool &isGroup, int group)
{
    AddressGroup gr;
    int distance = 0;
...
bool OLSRPO::getNextHopGroup(const L3Address& dest, L3Address &next, int &iface, L3Address& gw, bool &isGroup, int group)
{
    AddressGroup gr;
    bool find = false;
    if (findInAddressGroup(dest, group))
...
L3Address OLSRPO::getIfaceAddressFromIndex(int index)
{
...
void OLSRPO::getDistributionPath(const L3Address &addr,std::vector<L3Address> &path)
{
...
bool OLSRPO::handleNodeStart(IDoneCallback *doneCallback)
{
    hello_timer_.resched(hello_ival());
    tc_timer_.resched(hello_ival());
    mid_timer_.resched(hello_ival());
...
bool OLSRPO::handleNodeShutdown(IDoneCallback *doneCallback)
{
