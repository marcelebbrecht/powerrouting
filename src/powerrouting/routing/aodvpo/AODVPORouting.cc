...
typedef inet::NodeStatus NodeStatus;
typedef inet::IPSocket IPSocket;
typedef inet::ICMPMessage ICMPMessage;
typedef inet::INetworkProtocolControlInfo INetworkProtocolControlInfo;
typedef inet::UDPPacket UDPPacket;
typedef inet::IdealMacFrame IdealMacFrame;
typedef inet::CSMAFrame CSMAFrame;
typedef inet::CsmaCaMacFrame CsmaCaMacFrame;
typedef inet::LMacFrame LMacFrame;
typedef inet::BMacFrame BMacFrame;
typedef inet::XMacPkt XMacPkt;
typedef inet::INetworkDatagram INetworkDatagram;
typedef inet::NodeStartOperation NodeStartOperation;
typedef inet::NodeShutdownOperation NodeShutdownOperation;
typedef inet::NodeCrashOperation NodeCrashOperation;
typedef inet::ieee80211::Ieee80211Frame Ieee80211Frame;
...
void AODVPORouting::initialize(int stage)
{
    if (stage == inet::INITSTAGE_LOCAL) {
        ...
        routingTable = inet::getModuleFromPar<IRoutingTable>(par("routingTableModule"), this);
        interfaceTable = inet::getModuleFromPar<IInterfaceTable>(par("interfaceTableModule"), this);
        networkProtocol = inet::getModuleFromPar<INetfilter>(par("networkProtocolModule"), this);
        ...
        waitForRREP = new cMessage("waitForRREP");
...
//            throw cRuntimeError("Model error: self msg '%s' received when isOperational is false", msg->getName());
    ...
    if (msg->isSelfMessage()) {
        if (dynamic_cast<WaitForRREP *>(msg))
            ...
            expungeRoutes();
        else if (msg == waitForRREP) {
            sendAODVPacket(waitPacket, waitAddress, waitTimer, 0);
            EV_INFO << "Sending Route Reply now" << endl;
        }
...
INetfilter::IHook::Result AODVPORouting::ensureRouteForDatagram(INetworkDatagram *datagram)
{
...
bool AODVPORouting::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void AODVPORouting::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address AODVPORouting::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void AODVPORouting::delayDatagram(INetworkDatagram *datagram)
{
...
void AODVPORouting::sendRREQ(AODVRREQ *rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void AODVPORouting::sendRREP(AODVRREP *rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
AODVRREP *AODVPORouting::createRREP(AODVRREQ *rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
...
AODVRREP *AODVPORouting::createGratuitousRREP(AODVRREQ *rreq, IRoute *originatorRoute)
{
...
void AODVPORouting::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void AODVPORouting::sendAODVPacket(AODVControlPacket *packet, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
    ...
    networkProtocolControlInfo->setTransportProtocol(inet::IP_PROT_MANET);
...
void AODVPORouting::handleRREQ(AODVRREQ *rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
...
IRoute *AODVPORouting::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void AODVPORouting::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
    ...
    if (signalID == inet::NF_LINK_BREAK) {
...
void AODVPORouting::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
void AODVPORouting::handleRERR(AODVRERR *rerr, const L3Address& sourceAddr)
{
...
void AODVPORouting::forwardRREP(AODVRREP *rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void AODVPORouting::forwardRREQ(AODVRREQ *rreq, unsigned int timeToLive)
{
...
void AODVPORouting::completeRouteDiscovery(const L3Address& target)
{
...
void AODVPORouting::sendGRREP(AODVRREP *grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
AODVRREP *AODVPORouting::createHelloMessage()
{
...
void AODVPORouting::handleHelloMessage(AODVRREP *helloMessage)
{
...
INetfilter::IHook::Result AODVPORouting::datagramForwardHook(INetworkDatagram *datagram, const InterfaceEntry *inputInterfaceEntry, const InterfaceEntry *& outputInterfaceEntry, L3Address& nextHopAddress)
{
    ...
    if ( powerTriggerLast < newTrigger ) {
        ...
        sendRERRWhenNoRouteToForward(getSelfIPAddress());
        for (int i = 0; i < routingTable->getNumRoutes(); i++) {
            IRoute *route = routingTable->getRoute(i);
...
void AODVPORouting::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
    ...
    rerrCount++;
    EV_INFO << "Broadcasting Route Error message with TTL=32" << endl;
    sendAODVPacket(rerr, addressType->getBroadcastAddress(), 32, jitterPar->doubleValue());    // TODO: unicast if there exists a route to the source
}

void AODVPORouting::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool AODVPORouting::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AODVRouteData *routeData = check_and_cast<AODVRouteData *>(route->getProtocolData());
...
AODVRREPACK *AODVPORouting::createRREPACK()
{
...
void AODVPORouting::sendRREPACK(AODVRREPACK *rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void AODVPORouting::handleRREPACK(AODVRREPACK *rrepACK, const L3Address& neighborAddr)
{
